...
p_array = linspace(0, 1 , 5);

H_array = zeros(1, length(p_array));
Z_array = zeros(1, length(p_array));
H_p_array = H_array;
Z_p_array = Z_array;


for count = 1:length(p_array)
    ...
    proportion = p;
    H_pattern = 0.5 * (rand(n_row, n_col) < proportion);


    effective_tiles = H_pattern > 0;
    L_pattern = ones(n_row, n_col) * 0.75;
    T_pattern = ones(n_row, n_col) * 29;
...
%     sgtitle('L-thres = ' + string(params.ROS.L_thres))
    ...
    H_array(count) = H_sum(end);
    Z_array(count) = Z_sum(end);
    H_p_array(count) = H_sum(end)/n_effective;
    Z_p_array(count) = Z_sum(end)/n_effective;
